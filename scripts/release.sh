#!/usr/bin/env bash
# Release script for ccgo-gradle-plugins
# Usage: ./scripts/release.sh <version> [--push]
#        ./scripts/release.sh --patch|--minor|--major [--push]
#
# Examples:
#   ./scripts/release.sh 1.0.0
#   ./scripts/release.sh --patch        # 0.0.20 -> 0.0.21
#   ./scripts/release.sh --minor        # 0.0.20 -> 0.1.0
#   ./scripts/release.sh --major        # 0.0.20 -> 1.0.0
...
get_current_version() {
	grep '^VERSION_NAME=' gradle.properties | sed 's/VERSION_NAME=//'
}
...
update_version_in_file() {
	...
	if [[ -f "$file" ]]; then
		...
		if [[ "$OSTYPE" == "darwin"* ]]; then
			sed -i '' "s/^VERSION_NAME=$old_version/VERSION_NAME=$new_version/" "$file"
		else
			sed -i "s/^VERSION_NAME=$old_version/VERSION_NAME=$new_version/" "$file"
		fi
...
main() {
	if [[ $# -eq 0 ]] || [[ "$1" == "-h" ]] || [[ "$1" == "--help" ]]; then
		...
		echo ""
		echo "Options:"
		echo "  <version>   Specific version (e.g., 1.0.0)"
		echo "  --patch     Bump patch version (0.0.20 -> 0.0.21)"
		echo "  --minor     Bump minor version (0.0.20 -> 0.1.0)"
		echo "  --major     Bump major version (0.0.20 -> 1.0.0)"
	...
	echo -e "${GREEN}Releasing ccgo-gradle-plugins${NC}"
	...
	update_version_in_file "gradle.properties" "$current_version" "$new_version"
	...
	echo "  2. Commit: git add gradle.properties && git commit -m 'chore: release v$new_version'"
	echo "  3. Tag: git tag v$new_version"
	echo "  4. Push: git push && git push origin v$new_version"
	echo ""
	echo "One-liner:"
	echo "  git add gradle.properties && git commit -m 'chore: release v$new_version' && git tag v$new_version && git push && git push origin v$new_version"
	echo ""
